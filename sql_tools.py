"""
sql_tools.py — Herramientas LangChain para el agente analista SQL.

Se exponen cuatro herramientas:
    1. list_tables    — Lista tablas disponibles en la BD
    2. describe_table — Schema, recuento de filas y muestra
    3. run_sql        — Ejecuta consultas SELECT y devuelve resultados
    4. generate_chart — Genera gráficas matplotlib con conn, pd, plt inyectados

Teaching points:
    - check_same_thread=False en sqlite3.connect(): Streamlit puede invocar
      callbacks desde hilos distintos al principal; sin esto SQLite lanza
      ProgrammingError.
    - run_sql solo acepta SELECT: validación simple pero suficiente para
      proteger contra escrituras accidentales del LLM.
    - generate_chart inyecta `conn` en lugar de `df`. El LLM usa
      pd.read_sql(query, conn) para cargar datos antes de graficarlos.

Public API
----------
    build_sql_tools(db_path) -> list[BaseTool]
"""
...
def build_sql_tools(db_path: str) -> list[Any]:
    """
    Construye y devuelve las herramientas SQL para el agente.

    Parameters
    ----------
    db_path : str
        Ruta al archivo SQLite (.db).
    ...
    """
    # check_same_thread=False: Streamlit puede ejecutar en hilos distintos
    conn = sqlite3.connect(db_path, check_same_thread=False)

    # ------------------------------------------------------------------
    # Herramienta 1: list_tables
    # ------------------------------------------------------------------
    @tool
    ...
    def describe_table(table_name: str) -> str:
        ...
        Usa esta herramienta antes de escribir SQL para conocer los nombres
        exactos de columnas y los tipos de datos disponibles.
    ...
    def run_sql(query: str) -> str:
        ...
        Funciones útiles en SQLite:
            ...
            strftime('%Q', date)     — trimestre (no existe en SQLite, usar:)
            CASE WHEN strftime('%m',date) <= '03' THEN 'Q1' ... END
    ...
    # ------------------------------------------------------------------
    # Igual que en tools.py pero con `conn` en lugar de `df`.
    # El LLM debe usar pd.read_sql(query, conn) para cargar datos.
    # ------------------------------------------------------------------
    @tool
